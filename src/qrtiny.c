...
#define QRTINY_FORMATINFO_TO_ECL(_v) \
    ((((_v) ^ QRTINY_FORMATINFO_MASK) >> (QRTINY_SIZE_BCH + QRTINY_SIZE_MASK)) & ((1 << QRTINY_SIZE_ECL) - 1))
#define QRTINY_FORMATINFO_TO_MASKPATTERN(_v) \
    ((((_v) ^ QRTINY_FORMATINFO_MASK) >> (QRTINY_SIZE_BCH)) & ((1 << QRTINY_SIZE_MASK) - 1))
...
#define QRTINY_MODE_INDICATOR_NUMERIC \
    0x1 // 0b0001 Numeric (maximal groups of 3/2/1 digits encoded to 10/7/4-bit binary)
#define QRTINY_MODE_INDICATOR_ALPHANUMERIC \
    0x2 // 0b0010 Alphanumeric ('0'-'9', 'A'-'Z', ' ', '$', '%', '*', '+', '-', '.', '/', ':') -> 0-44 index. Pairs
        // combined (a*45+b) encoded as 11-bit; odd remainder encoded as 6-bit.
...
#define QRTINY_SEGMENT_NUMERIC_BUFFER_BITS(_c)                                                            \
    (QRTINY_SIZE_MODE_INDICATOR + QRTINY_MODE_NUMERIC_COUNT_BITS + (10 * ((_c) / 3)) + (((_c) % 3) * 4) - \
     (((_c) % 3) / 2))
#define QRTINY_SEGMENT_ALPHANUMERIC_BUFFER_BITS(_c) \
    (QRTINY_SIZE_MODE_INDICATOR + QRTINY_MODE_ALPHANUMERIC_COUNT_BITS + 11 * ((_c) >> 1) + 6 * ((_c) & 1))
...
size_t QrTinyWriteNumeric(void* buffer, size_t bitPosition, const char* text)
{
    ...
    size_t i;
    bitsWritten += QrTinyBufferAppend(buffer, bitPosition + bitsWritten, QRTINY_MODE_INDICATOR_NUMERIC,
                                      QRTINY_SIZE_MODE_INDICATOR);
    bitsWritten +=
        QrTinyBufferAppend(buffer, bitPosition + bitsWritten, (uint32_t) charCount, QRTINY_MODE_NUMERIC_COUNT_BITS);
    for (i = 0; i < charCount;) {
        ...
        if (remain > 1) {
            value  = value * 10 + text[i + 1] - '0';
            bits  += 3;
        }
        if (remain > 2) {
            value  = value * 10 + text[i + 2] - '0';
            bits  += 3;
        }
...
size_t QrTinyWriteAlphanumeric(void* buffer, size_t bitPosition, const char* text)
{
    ...
    size_t bitsWritten         = 0;
    size_t i;
    bitsWritten += QrTinyBufferAppend(buffer, bitPosition + bitsWritten, QRTINY_MODE_INDICATOR_ALPHANUMERIC,
                                      QRTINY_SIZE_MODE_INDICATOR);
    bitsWritten += QrTinyBufferAppend(buffer, bitPosition + bitsWritten, (uint32_t) charCount,
                                      QRTINY_MODE_ALPHANUMERIC_COUNT_BITS);
    for (i = 0; i < charCount;) {
...
size_t QrTinyWrite8Bit(void* buffer, size_t bitPosition, const char* text)
{
    ...
    size_t i;
    bitsWritten +=
        QrTinyBufferAppend(buffer, bitPosition + bitsWritten, QRTINY_MODE_INDICATOR_8_BIT, QRTINY_SIZE_MODE_INDICATOR);
    bitsWritten +=
        QrTinyBufferAppend(buffer, bitPosition + bitsWritten, (uint32_t) charCount, QRTINY_MODE_8BIT_COUNT_BITS);
...
static size_t QrTinyIdentifyIndex(int x, int y)
{
    ...
    int bit;
    if (h < 4)
        module = dir ? h * 3 + v : h * 3 + 2 - v;
    else if (h < 6)
        module = 12 + (dir ? (h - 4) * 5 + v : (h - 4) * 5 + 4 - v);
    else
        module = 22 + h - 6;
...
static int QrTinyIdentifyModule(int x, int y, uint16_t formatInfo)
{
    ...
    // Quiet zone
    if (x < 0 || y < 0 || x >= QRTINY_DIMENSION || y >= QRTINY_DIMENSION) {
        return QRTINY_MODULE_LIGHT;
    } // Outside
    ...
    // Timing
    if (x == QRTINY_TIMING_OFFSET || y == QRTINY_TIMING_OFFSET) {
        return ((x ^ y) & 1) ? QRTINY_MODULE_LIGHT : QRTINY_MODULE_DARK;
    } // Timing: vertical
    ...
    if (x == QRTINY_FINDER_SIZE + 1 && y == QRTINY_DIMENSION - QRTINY_FINDER_SIZE - 1) {
        return QRTINY_MODULE_DARK;
    } // Always-black module, right of bottom-left finder
    if (xx <= QRTINY_FINDER_SIZE && yy <= QRTINY_FINDER_SIZE) {
        formatIndex = 7 - xx + yy;
    }
    if (x == QRTINY_FINDER_SIZE + 1 && y >= QRTINY_DIMENSION - QRTINY_FINDER_SIZE - 1) {
        formatIndex = y + 14 - (QRTINY_DIMENSION - 1);
    } // Format info (right of bottom-left finder)
    if (y == QRTINY_FINDER_SIZE + 1 && x >= QRTINY_DIMENSION - QRTINY_FINDER_SIZE - 1) {
        formatIndex = QRTINY_DIMENSION - 1 - x;
    } // Format info (bottom of top-right finder)
    if (formatIndex >= 0) {
...
// This error-correction calculation derived from https://www.nayuki.io/page/qr-code-generator-library Copyright (c)
// Project Nayuki. (MIT License)
static void QrTinyRSRemainder(const uint8_t data[], size_t dataLen, const uint8_t generator[], int degree,
                              uint8_t result[])
{
    size_t i;
    int j;
...
// [Table 13] Number of error correction codewords (count of data 8-bit codewords in each block; for each
// error-correction level in V1)
...
static const uint8_t eccDivisorsMedium[]                = {0xd8, 0xc2, 0x9f, 0x6f, 0xc7,
                                                           0x5e, 0x5f, 0x71, 0x9d, 0xc1}; // V1 0b00 Medium ECL
static const uint8_t eccDivisorsLow[]                   = {0x7f, 0x7a, 0x9a, 0xa4, 0x0b, 0x44, 0x75}; // V1 0b01 Low ECL
static const uint8_t eccDivisorsHigh[]                  = {0x77, 0x42, 0x53, 0x78, 0x77, 0x16, 0xc5, 0x53, 0xf9,
                                                           0x29, 0x8f, 0x86, 0x55, 0x35, 0x7d, 0x63, 0x4f}; // V1 0b10 High ECL
static const uint8_t eccDivisorsQuartile[]              = {0x89, 0x49, 0xe3, 0x11, 0xb1, 0x11, 0x34,
                                                           0x0d, 0x2e, 0x2b, 0x53, 0x84, 0x78}; // V1 0b11 Quartile ECL
static const uint8_t* eccDivisors[1 << QRTINY_SIZE_ECL] = {eccDivisorsMedium, eccDivisorsLow, eccDivisorsHigh,
                                                           eccDivisorsQuartile};
...
bool QrTinyGenerate(uint8_t* buffer, size_t payloadLength, uint16_t formatInfo)
{
    ...
    int eccCodewords =
        qrcode_ecc_block_codewords[errorCorrectionLevel]; // (sizeof(eccDivisors[errorCorrectionLevel]) /
                                                          // sizeof(eccDivisors[errorCorrectionLevel][0]));
...
#define QRTINY_PAD_CODEWORDS 0xec11 // Pad codewords 0b11101100=0xec 0b00010001=0x11
    ...
    QrTinyRSRemainder(buffer, dataCapacity / 8, eccDivisor, eccCodewords,
                      buffer + (QRTINY_TOTAL_CAPACITY - ((size_t) 8 * eccCodewords)) / 8);
    return true;
}
